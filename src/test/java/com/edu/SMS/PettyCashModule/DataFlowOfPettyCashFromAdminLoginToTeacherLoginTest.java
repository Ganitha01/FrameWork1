...
import com.edu.ObjectRepository.TeacherDashBoardPreviewPage;
	...
	public class DataFlowOfPettyCashFromAdminLoginToTeacherLoginTest extends BaseClass {
			...
			int num=jutil.getRandomNumber();
				...
				String TeacherUrl=driver.getCurrentUrl();
					...
					String adminPettyCashDetails=apcpPage.fetchPettyCashDetails(TEACHERNAME+num, wutil, "100", PettyCashDes, AMOUNT);
						...
						if(adminPettyCashDetails.contains(PettyCashDes)&&adminPettyCashDetails.contains(AMOUNT))
						{
							 apcpPage =  apcpPage.approvePettyCash(wutil,driver);
							System.out.println("petty cash approved successfully");
						}
						//log out as a admin
						adpPage.signOutAdminLogin(wutil, driver);
			
						//login to the application as a teacher
						...
						tdpPage.clickOnMyPettyCashLnk();
						String paidDetails=tppPage.fetchPaidDetails(driver,CURRENTDATE, wutil, "100",PettyCashDes, AMOUNT);
						
					Assert.assertTrue(paidDetails.contains(AMOUNT));
							Reporter.log("Petty cash is approved",true);
