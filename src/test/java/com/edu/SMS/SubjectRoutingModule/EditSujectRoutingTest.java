...
public class EditSujectRoutingTest extends BaseClass{
	...
	public void createAndEditSubjectRouting() throws Throwable{
		...
		String MODIFIEDTEACHERAMOUNT=eutil.fetchDataFromExcel("SubjectRoutin", 4, 15);
		//create teacher
	...
	//enter teacher name in search bar
		...
		Assert.assertEquals(createdGrade.equalsIgnoreCase(GRADE),true);
		...
		//modify the subject routing
		srPage.editPettyCash(TEACHERNAME, MODIFIEDTEACHERAMOUNT,wutil,driver);
		//System.out.println("subject routing modified successfully");
		//fetch updated fee
		//adbpPage.clickOnSubjectRoutingLnk(wutil,driver);
		String updatedFee=srPage.fetchingupdatedFee(wutil,driver,TEACHERNAME,MODIFIEDTEACHERAMOUNT);
		//System.out.println(updatedFee+""+MODIFIEDTEACHERAMOUNT);
		Assert.assertTrue(updatedFee.contains(MODIFIEDTEACHERAMOUNT));
		Reporter.log("Subject routing edited successfully",true);
